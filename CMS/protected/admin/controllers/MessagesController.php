...
class MessagesController extends Controller
{
	...
	public function accessRules()
	{
		return array(
			...
			array('allow', // allow authenticated user to perform 'create' and 'update' actions
				'actions'=>array('create','update','markAsRead'),
				'users'=>array('@'),
			),
	...
	public function actionIndex()
	{
		$unread_messages = Yii::app()->db->createCommand()
				...
				->queryAll();
		$read_messages = Yii::app()->db->createCommand()
				->select()
				->from('messages')
				->where('read=:read', array(':read'=>'TRUE'))
				->queryAll();
		$all_messages = Yii::app()->db->createCommand()
				->select()
				->from('messages')
				->queryAll();
	...
	public function actionMarkAsRead($id, $mark)
	{
		echo $mark;
		$model=$this->loadModel($id);
		if($model!=null){
